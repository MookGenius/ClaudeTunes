...
# -*- coding: utf-8 -*-
"""
GT7 Analyzer v3 - COMPLETE FIX
- Auto-classifies car type from telemetry
- Adaptive thresholds per car category
- Data cleaning and validation
- Fixed field name mismatches
- Removed G-force analysis (Packet A doesn't have it)
...
def process_session_folder(session_folder):
    ...
    output_file = os.path.join(session_folder, 'telemetry_analysis_v3.json')
...
def process_single_lap(lap_file, thresholds, total_stats):
    ...
    try:
        ...
        lap_metrics = analyze_lap_data(validated_data, thresholds)
        ...
        lap_metrics['data_points_analyzed'] = len(validated_data)
    ...
    metrics = {
        ...
        'tire_analysis': {
            'temperatures': {
                ...
                'rl_min': mn('tire_temp_rl'), 'rr_min': mn('tire_temp_rr'),
...
def main():
    ...
    print(f"GT7 Telemetry Analyzer v3 - FIXED & ENHANCED")
    ...
    if overall:
        ...
        print(f"  Apex Speed Consistency: {overall['apex_score']:.1f}/100")
        print(f"  Braking Consistency: {overall['braking_score']:.1f}/100")
        print(f"  Lap time delta: {cons['lap_time_consistency']['delta']:.3f}s")
    else:
    ...
    if stint:
        ...
        print(f"  Most degraded: {stint['most_degraded_tire'].upper()}")
        print(f"  Least degraded: {stint['least_degraded_tire'].upper()}")
        if 'lap_time_delta' in stint:
            print(f"  Time lost over stint: +{stint['lap_time_delta']:.3f}s")
            print(f"  Avg time lost per lap: +{stint['time_lost_per_lap']:.3f}s")
    else:
    ...
    print(f"  Front avg slip: {balance['front_avg_slip']:.4f}")
    print(f"  Rear avg slip: {balance['rear_avg_slip']:.4f}")
    print(f"  Balance metric: {balance['balance_metric']:+.4f}")
    print(f"  Threshold used: >{slip['threshold_used']}")
    if slip['recommendations']:
        print(f"\n  💡 Setup Recommendations:")
        for rec in slip['recommendations']:
            print(f"     • {rec}")

    print(f"\n🔧 SUSPENSION")
    susp = result['suspension_summary']
    print(f"  Avg body height: {susp['avg_body_height']:.1f}mm")
    print(f"  Lowest body height: {susp['lowest_body_height']:.1f}mm")
    print(f"  Corner compression avg:")
    print(f"    FL: {susp['corner_compression_avg']['fl']:.4f}  FR: {susp['corner_compression_avg']['fr']:.4f}")
    print(f"    RL: {susp['corner_compression_avg']['rl']:.4f}  RR: {susp['corner_compression_avg']['rr']:.4f}")

    print(f"\n🌡️  TIRE TEMPERATURES")
    temps = result['tire_summary']['temperature_averages']
    print(f"  FL: {temps['fl']:.1f}°C  FR: {temps['fr']:.1f}°C")
    print(f"  RL: {temps['rl']:.1f}°C  RR: {temps['rr']:.1f}°C")
